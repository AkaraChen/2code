...
我认真读了这些页面，也觉得这个方向是对的——agent 的边际成本确实在往下掉。Superset 那篇路线图里有句很清醒的话：*"Agent compute is already cheap enough, you can run hundreds of agents a month all for less than the cost of one engineer."*

所以这篇不是拆台。我只是要说清楚：作为一个做工具的独立开发者，我**为什么不先做那一层**。
...
也就是说：一家专门做并行编排的公司，今天的稳定并发是 **5–7**，而且他们的 agent **大部分时间在等人 review**。100 是目标，不是现状——他们自己就是这么写的。

那你呢？

别拍脑袋，去数一下。数你上周真正**同时在跑、并且真的跟进到底**的任务线有几条。不是「我开过多少个终端窗口」，是「有几条线我最后真的看了 diff、做了决定」。

我自己数出来的量级是个位数：日常 2 到 3 条，忙的时候多一两条。2code 就是照着这个量级设计的。
...
所以「百 Agent 工厂」是一个团队级的目标，它得把 review 也自动化掉才成立。它很值得做，但它不是我今天要解决的问题。
...
**二、通知。** Agent 跑完了，谁告诉我？没人的话，我就变成了轮询器：每两分钟切一圈窗口看看谁好了。三条线的轮询成本，已经足够让我不敢离开桌子——而「让 agent 干活的时候我去干别的」，本来才是并行的全部意义。
...
这四件事有个共同点：**它们跟你并行几条线没关系，跑一条线也一样疼。** 加到 100 条只会更疼，但根源不在数量。
...
- **持久终端 + 恢复。** 终端会话、scrollback、窗口布局，重启后恢复。回来看到的是离开时的样子。
...
我知道这份清单看起来「少」。但少和取舍是两回事——上面每一条「没做」，都是因为它服务的是另一个层级的问题，而不是因为还没排上期。

顺带说一句我很喜欢的参照：JetBrains 给 Fleet 收尾的那篇文章，最有价值的部分不是讲他们要做什么，而是讲他们**决定不做什么**。工具的诚实度，往往就体现在这份清单上。
...
- 你想要 25+ 个 agent 开箱即用地摆在一起，随时对比。

**用 2code，如果：**

- 你的真实并发是 2–5 条，剩下的痛点是恢复、通知和 review 距离。
- 你想要一个每天都开着的工作台，而不是一套要先配置好流程才能开始的系统。
- 你重度用终端，希望 CLI agent 就在它本来该在的地方跑，而不是被塞进某个侧边栏。

这两件事不互斥。工作台是底座，编排是上层。**底座晃的时候，上层的收益会打折**——50 个 agent 帮你跑起来了，可你桌面上这三条到底谁跑完了，还是得有人告诉你。

等到有一天，我自己的真实并发从 3 条涨到 30 条，我会认真去想上面那层怎么做。在那之前，承诺 100 是不诚实的。

## 先把三条泳道用顺

如果你数完自己的真实并发，发现也是两三条，而每天真正难受的，是重启后要重新考古、是不敢离开桌子、是 diff 离你太远——那我们要解决的是同一个问题。
...
2code 还很早期，也还在动。但它承诺的东西很小，也很具体：你今天真实的那两三条线，不用你操心。
