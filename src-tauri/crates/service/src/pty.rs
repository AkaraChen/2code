...
const PERSIST_BATCH_INTERVAL: Duration = Duration::from_millis(100);
...
/// Persistence thread: batches raw bytes and flushes them to DB on size or time thresholds.
fn persist_pty_output(
	...
	loop {
		match rx.recv_timeout(PERSIST_BATCH_INTERVAL) {
			Ok(PersistMsg::Data(data)) => {
				pending.extend_from_slice(&data);
				if pending.len() >= PERSIST_BATCH_BYTES {
					flush_persist_buffer(&db, session_id, &mut pending);
				}
			}
			Ok(PersistMsg::Flush) => {
				flush_persist_buffer(&db, session_id, &mut pending);
...
fn flush_persist_buffer(db: &DbPool, session_id: &str, pending: &mut Vec<u8>) {
	if pending.is_empty() {
		return;
	}
	...
	match repo::pty::append_output(&mut conn, session_id, &data) {
		Ok(()) => {
			tracing::info!(target: "pty", %session_id, n, "persist: appended batch");
		}
		Err(e) => {
			tracing::warn!(target: "pty", %session_id, n, error = %e, "persist: failed to append batch");
			*pending = data;
		}
	}
}
...
mod tests {
	...
	fn build_benchmark_history(target_bytes: usize) -> Vec<u8> {
		let mut raw = Vec::with_capacity(target_bytes + 256);
		let mut line = 0usize;

		while raw.len() < target_bytes {
			raw.extend_from_slice(
				format!(
					"\x1b[32m[{line:06}]\x1b[0m cargo test --target mock-{line:06}\r\n"
				)
				.as_bytes(),
			);
			...
			if line % 7 == 0 {
				raw.extend_from_slice(
					format!("\r\x1b[2Kprogress {:>3}% ", line % 100).as_bytes(),
				);
			}

			if line % 13 == 0 {
				raw.extend_from_slice(
					b"\x1b[33mwarning:\x1b[0m benchmark marker\r\n",
				);
			}
	...
	#[test]
	#[ignore = "benchmark"]
	fn benchmark_sanitize_history_restore_payloads() {
		let sizes = [
			64 * 1024,
			256 * 1024,
			512 * 1024,
			repo::pty::PERSISTED_PTY_OUTPUT_BYTES_CAP,
			2 * repo::pty::PERSISTED_PTY_OUTPUT_BYTES_CAP,
		];

		for size in sizes {
			let raw = build_benchmark_history(size);
			let mut samples = Vec::new();
			let mut clean_len = 0usize;

			for _ in 0..5 {
				let started = std::time::Instant::now();
				let sanitized = std::hint::black_box(sanitize_history(
					std::hint::black_box(raw.as_slice()),
					24,
					120,
				));
				let elapsed = started.elapsed();
				clean_len = sanitized.len();
			...
			let avg_ms = samples
				.iter()
				.map(|sample| duration_to_ms(*sample))
				.sum::<f64>()
				/ samples.len() as f64;
			let max_ms = samples
				.iter()
			...
			eprintln!(
				"[bench][sanitize_history] raw_bytes={} clean_bytes={} min_ms={:.2} avg_ms={:.2} max_ms={:.2}",
				raw.len(),
				clean_len,
				min_ms,
				avg_ms,
			...
			assert!(clean_len > 0, "sanitized output should not be empty");
		}
	}
}
